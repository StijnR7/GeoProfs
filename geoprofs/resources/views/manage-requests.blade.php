...
<head>
    ...
    <title>Beheer Aanvragen - Geoprofs</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-approve { padding: 6px 12px; background: #27ae60; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 12px; transition: background 0.3s; }
        .btn-approve:hover { background: #229954; }
        .btn-reject { padding: 6px 12px; background: #e74c3c; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 12px; transition: background 0.3s; }
...
<body>
    ...
    <div class="nav-buttons">
        ...
        @if(Auth::user()->functie === 'admin')
            <a href="{{ route('manage-requests') }}" class="nav-btn active">Beheer Aanvragen</a>
        @endif
    </div>
    ...
    <div class="container">
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <form action="{{ route('leave.reject', $leave->id) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('POST')
                                        <button type="submit" class="btn-reject">Afwijzen</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
        ...
        @else
            <div class="no-requests">
                <p>Er zijn geen openstaande aanvragen om te beoordelen.</p>
            </div>
        @endif
    </div>
